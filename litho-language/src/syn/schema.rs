...
{
    many0(type_system_definition())
        .map(|definitions| TypeSystemDocument { definitions })
        .boxed()
}
...
where
    ...
    T: for<'b> PartialEq<&'b str> + Clone + 'a,
{
    string_value().map(Description).boxed()
}
    ...
    T: for<'b> PartialEq<&'b str> + Clone + 'a,
{
    opt(description())
        ...
        .and(ty(RECURSION_LIMIT).recover(Missing::unary(
